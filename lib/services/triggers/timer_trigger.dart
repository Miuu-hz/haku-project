  ...
  Future<void> _fireTrigger(PendingTrigger trigger) async {
    ...
    switch (trigger.type) {
      case TimerTriggerType.locationFollowUp:
        final locationName = (trigger.data?['locationName'] as String?) ?? 'ที่นี่';
        message = 'ร้าน$locationNameเป็นไงบ้างคะ? อร่อยไหม? 😊';
        break;

      case TimerTriggerType.morningReminder:
        message = (trigger.data?['message'] as String?) ?? 'สวัสดีตอนเช้าค่ะ!';
        break;

      case TimerTriggerType.healthCheck:
        message = (trigger.data?['message'] as String?) ?? 'วันนี้อาการเป็นไงบ้างคะ?';
        break;

      case TimerTriggerType.goalReminder:
        final goal = (trigger.data?['goal'] as String?) ?? 'เป้าหมาย';
        message = 'อย่าลืม$goalนะคะ! 💪';
        break;

      case TimerTriggerType.custom:
        message = (trigger.data?['message'] as String?) ?? '';
        break;
    }
